...
const WordCounter = () => {
  ...
  const zhCompleted = useSelector(
    (state) => state.typingtest.statistics.zhCompleted
  );
  const testLanguage = useSelector(
    (state) => state.typingtest.options.language
  );
  ...
  const quoteWordCount = useSelector(
    (state) => state.typingtest.loading.quoteWordCount
  );
